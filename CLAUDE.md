...
`evals/manifest.json` defines 36 comparison cases (`pseudo-etymology`, `register-shift`, `source-claim`, cluster/adversarial cases, plus three deterministic Sanskrit cases `translit-mixed-scheme`/`translit-first-mention`/`gost-hallucinated-ref`) that map to documents under `examples/input/`. The deterministic cases are gated on the transliteration linter and citation grounding (which `run_eval_case` runs as provider-independent post-verification checks via `_run_deterministic_checks`), so they pass identically on `mock`. `rws eval-suite --provider mock --suite-id <id>` runs all cases and writes `eval-suite-result.json` + `eval-suite-report.md`. Use `rws eval-compare A B` to diff two suite runs; `--strict` makes regressions fail with exit 1. The `Eval Smoke` GitHub Actions workflow pins this for the mock provider.
