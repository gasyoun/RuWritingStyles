...
1. Добавить `styles/manifest.yml`.
2. Создать паспорта для трех MVP-стилей.
3. Добавить `model_policy.yml` с ролями моделей и reasoning-режимами.
4. Описать JSON-схему замечания.
5. Написать минимальный CLI для `.md`-документов.
6. Реализовать `rws review` для одного стиля.
