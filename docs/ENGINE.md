...
*   **Eval-дисциплина**: `evals/manifest.json` содержит 33 кейса; `mock` provider используется для инфраструктурного smoke, реальные провайдеры сравниваются через `eval-suite` и `eval-compare`. Статистически осмысленный бенчмарк — `eval-run --repeat N` / `eval-suite --repeat N` (усреднённый агрегат pass-rate / detection-rate ± σ), см. [`docs/benchmark.md`](https://github.com/gasyoun/RuWritingStyles/blob/main/docs/benchmark.md).
