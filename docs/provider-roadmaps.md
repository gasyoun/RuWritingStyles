...
Каждый провайдер должен пройти один и тот же набор. Базовый manifest сейчас содержит 52 eval-кейса; для дорогих real-provider проверок можно запускать меньший tagged subset, но promoted baseline должен быть сопоставим с полным набором.
