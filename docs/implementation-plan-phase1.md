...
Дата: 2026-06-12. Детализация фазы 1 из [roadmap-sanskrit-dh.md](https://github.com/gasyoun/RuWritingStyles/blob/main/docs/roadmap-sanskrit-dh.md).
...
Каждый блок работ (W1–W6) самодостаточен и завершается зеленым CI
...
3. **`latex.py`**: в `LATEX_TEMPLATE` добавить секцию `\section*{Литература}`,
   заполняемую из `format_gost`; BibTeX остается параллельным выходом для
...
Приемка: запись Зализняка 2004 в выводе побайтно совпадает с эталоном ГОСТ из теста.
...
3. **Интеграция**:
   ...
   - CLI: `rws run --lint-translit` (argparse в `cli.py::build_parser`,
     блок `run`-сабпарсера, строки ~111–157) — по умолчанию **включен**, выключение
...
`сло́во` не считаются IAST, елка/ѣ не ломают детектор (фикстуры из
`test_cli_pipeline.py::test_normalization_preserves_russian_philological_marks`).

Приемка: на чистом тексте 0 находок; на каждом подсаженном дефекте ≥1 находка
нужного типа с верным `span_id`.

## W3. Профиль журнала (≈1–2 дня)

`/.rws-project/project-context.json` сейчас несет только `stylistic_commitments`
...
- Вынести шаг `reports` в общую функцию (`report.py::write_all_reports(run_dir)`),
  вызвать ее и из `cmd_run`, и из `run_full_pipeline`;
...
- **Эвристика Harvard-Kyoto дает ложные срабатывания** на английских вкраплениях
