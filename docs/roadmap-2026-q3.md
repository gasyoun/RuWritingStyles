...
- [x] Новые русские indology-журнальные профили сверх vya/ppv/vestnik-spbu (кандидаты:
      ...
      (Oriens)», «Вопросы языкознания») — **закрыто 30-07-2026 (H1883):**
      `knowledge/journals/vostok-oriens.json` (готов ранее) +
      [`knowledge/journals/iyakf.json`](https://github.com/gasyoun/RuWritingStyles/blob/main/knowledge/journals/iyakf.json)
      (новый, tronsky.iling.spb.ru/ru/policy.html) построены с нуля; третий кандидат,
      «Вопросы языкознания», уже существовал как `knowledge/journals/vya.json` с
      2026-06 (изначальный ГОСТ-плейсхолдер, никогда не сверенный с реальным сайтом) —
      теперь пересверен против vja.ruslang.ru/ru/author_guidelines и исправлен
      (реальный формат — не ГОСТ, а `[Автор год: стр.]`). Требования взяты с сайтов
      журналов, не выдуманы; провод в `report.journal_compliance` не менялся (та же
      схема `journal-profile.schema.json`).
