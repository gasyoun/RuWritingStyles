...
- **Measured before/after on the H072 N=5 flash protocol** ([`docs/benchmark.md`](docs/benchmark.md), 5 gold cases × N=5 on `deepseek-chat` → `deepseek-v4-flash`, Claude Fable 5 `claude-fable-5`, 2026-07-03): pass-rate **12/25 = 0.48 → 23/25 = 0.92**, diff-fidelity failures **13/25 → 0/25** (success criterion met: pass-rate ≈ detection-rate 0.96). Per-case: pseudo-etymology 3/5→5/5, karaka 1/5→5/5, vedic 3/5→4/5, samasa 4/5→4/5, commentary 1/5→5/5. The two remaining failures are a detection miss and one `failed` verification verdict — not revision fidelity.
