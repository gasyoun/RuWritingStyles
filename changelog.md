...
- **8 GOLD_DICTIONARY gold cases** (`dict-*`, [`evals/manifest.json`](evals/manifest.json) 44→52; inputs in [`examples/input/`](examples/input/) built on project-owned pwg_ru cards, ONE planted register violation each): circular gloss, publicistic tone, zone order, label discipline, grammar-zone prose, undiscriminated synonym heap, encyclopedic gloss, pseudo-etymology note. [`tools/gold_annotation.py`](tools/gold_annotation.py) gains `--tag` (default `GOLD_SANSKRIT` unchanged); protocol section added to [`evals/GOLD_PROTOCOL.md`](evals/GOLD_PROTOCOL.md). DeepSeek routed N=5 benchmark + two-rater κ annotation: numbers land in [`docs/benchmark.md`](docs/benchmark.md) with this release.
