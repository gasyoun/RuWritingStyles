...
class CliPipelineTests(unittest.TestCase):
    ...
    def test_full_mock_executed_run_updates_artifacts(self) -> None:
        ...
        self.assertEqual(len(provider_log_lines), len(load_manifest(ROOT).mvp_style_ids) + 4)
        ...
        self.assertIn(f"executions: {len(load_manifest(ROOT).mvp_style_ids) + 4}", provider_log_summary)
    ...
    def test_eval_suite_runs_manifest_cases(self) -> None:
        ...
        # The deterministic Sanskrit cases are gated on the transliteration linter
        # and citation grounding, so they pass identically under the mock provider;
        # every other case needs a real provider and fails on mock.
        mock_passing_case_ids = {
            "translit-mixed-scheme",
            "translit-first-mention",
            "gost-hallucinated-ref",
            "translit-inconsistent-rendering",
            "translit-cyrillic-latin-hybrid",
            "gost-broken-ref",
        }
