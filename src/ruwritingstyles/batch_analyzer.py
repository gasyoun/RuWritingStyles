...
def _process_cluster(cluster_id: str, repo_root: Path, input_file: Path, original_text: str, normalized_text: str, segments: list, manifest, model_policy, provider: str):
    print(f"\n>>> Profile: {cluster_id}")
...
def run_multi_style_batch(repo_root: Path, input_file: Path, clusters: list[str], provider: str):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(10, len(clusters))) as executor:
        futures = []
        for cluster_id in clusters:
            futures.append(executor.submit(
                _process_cluster, cluster_id, repo_root, input_file, original_text, normalized_text, segments, manifest, model_policy, provider
            ))
