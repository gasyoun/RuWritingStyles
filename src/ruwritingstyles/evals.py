...
def _make_aggregate_id(provider_name: str, kind: str) -> str:
    return make_unique_id(f"{kind}-{provider_name}")
...
def _make_suite_id(provider_name: str) -> str:
    return make_unique_id(f"eval-suite-{provider_name}")
...
def _suite_conditions_verdict(suite: dict[str, Any]) -> str:
    """H5097: per-suite integrity verdict for multi-suite pooling surfaces.

    ``clean`` — suite carries execution_conditions with no fallback events;
    ``fallback`` — suite carries execution_conditions but a fallback event
    poisoned comparability; ``unknown`` — suite predates execution_conditions
    (its provenance is simply not known, never assumed clean)."""
    conditions = suite.get("execution_conditions")
    if not isinstance(conditions, dict):
        return "unknown"
    if conditions.get("conditions_comparable") is False or conditions.get("fallback_events"):
        return "fallback"
    return "clean"
...
def generate_leaderboard_report(repo_root: Path, suite_paths: list[Path]) -> Path:
    ...
    md = [
        ...
        "",
        "| Model | Provider | Pass Rate | Match Count (Avg) | Fidelity (Avg) | Conditions |",
        "| :--- | :--- | :--- | :--- | :--- | :--- |"
    ]
    ...
    for path, data in suites:
        ...
        conditions = _suite_conditions_verdict(data)
        conditions_cell = {
            "clean": "clean",
            "fallback": "**fallback — do not pool**",
            "unknown": "unknown (predates execution_conditions)",
        }[conditions]
        ...
        md.append(f"| **{model}** | {provider} | {pass_rate} | {avg_match:.2f} | {avg_fidelity:.4f} | {conditions_cell} |")

    verdicts = [_suite_conditions_verdict(data) for _, data in suites]
    if any(verdict != "clean" for verdict in verdicts):
        md.append("")
        md.append(
            "**Pooling verdict: NO — do not pool.** "
            f"{verdicts.count('fallback')} suite(s) carry fallback events and "
            f"{verdicts.count('unknown')} suite(s) predate execution_conditions; "
            "cross-suite numbers are not execution-comparable."
        )

    md.append("\n*Generated by RuWritingStyles Multi-Agent Benchmark Suite*")

    report_path = repo_root / "runs" / f"{make_unique_id('leaderboard')}.md"
